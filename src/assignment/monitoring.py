...
class MonitoringAlert:
    ...
    def check_metrics(self) -> list[Alert]:
        ...
        if block_rate > self.block_rate_threshold:
            self.alerts.append(Alert(metric="block_rate", value=block_rate, threshold=self.block_rate_threshold, message="High block rate detected"))
            
        if self.rate_limit_hits > self.rate_limit_hit_threshold:
            self.alerts.append(Alert(metric="rate_limit_hits", value=self.rate_limit_hits, threshold=self.rate_limit_hit_threshold, message="High rate limit hits detected"))
            
        if judge_fail_rate > self.judge_fail_rate_threshold:
            self.alerts.append(Alert(metric="judge_fail_rate", value=judge_fail_rate, threshold=self.judge_fail_rate_threshold, message="High judge fail rate detected"))
